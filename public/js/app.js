...
var Message = React.createClass({
   ...
   render: function render() {
      ...
      return React.createElement(
         ...
         React.createElement('img', { src: 'images/chatprofile_icon.png', alt: 'profile' }),
         React.createElement(
            'div',
      ...
      return React.createElement(
         ...
         this.props.messages.map(function (message, i) {
            return React.createElement(Message, {
               ...
               user: message.user,
               text: message.text,
               isUserMessage: message.user === _this.props.currentUser
            });
         })
      );
   }
      ...
      return React.createElement(
         ...
         React.createElement(
            ...
            React.createElement(
               ...
               React.createElement('img', { src: 'images/send_btn.png', alt: 'Send' })
            )
         )
      );
   }
...
var SignUpForm = React.createClass({
   ...
   handleSubmit: function handleSubmit(e) {
      ...
      socket.emit('signup', { username: username, password: password }, function (response) {
         if (response.success) {
   ...
   render: function render() {
      return React.createElement(
         ...
         React.createElement(
            ...
            React.createElement(
               'button',
               { type: 'button', className: 'form_button', onClick: this.props.onSwitchToLogin },
               '로그인'
            )
         ),
   ...
   render: function render() {
      return React.createElement(
         ...
         React.createElement(
            ...
            React.createElement(
               'button',
               { className: 'form_button', type: 'submit' },
               '로그인'
            ),
            React.createElement(
               'button',
               { className: 'form_button', type: 'button', onClick: this.props.onSwitchToSignUp },
               '회원가입'
            )
         )
      );
...
var Header = React.createClass({
   ...
   render: function render() {
      return React.createElement(
         'div',
         { className: 'header' },
         React.createElement('img', { src: 'images/INU.png', alt: 'INU Logo', className: 'logo' })
      );
   }
});
...
var Drawer = React.createClass({
   ...
   render: function render() {
      return React.createElement(
         'div',
         { className: 'drawer' },
         React.createElement(DrawerItem, { icon: React.createElement('img', { src: 'images/friends_icon.png', alt: '친구' }) }),
         React.createElement(DrawerItem, { icon: React.createElement('img', { src: 'images/chat_icon.png', alt: '채팅' }) }),
         React.createElement(DrawerItem, { icon: React.createElement('img', { src: 'images/notification_icon.png', alt: '알림' }) }),
         React.createElement(DrawerItem, { icon: React.createElement('img', { src: 'images/profile_icon.png', alt: '프로필' }) })
      );
   }
});
...
var SearchRoom = React.createClass({
   ...
   handleSearch: function handleSearch() {
      ...
      socket.emit('search:room', { roomName: this.state.roomName }, function (response) {
         _this4.setState({ roomExists: response.exists });
         if (response.exists) {
            _this4.setState({ roomList: [_this4.state.roomName] });
         } else {
            _this4.setState({ roomList: [] });
         }
      });
   },
   ...
   handleCreateRoom: function handleCreateRoom() {
      ...
      if (this.state.roomName) {
         socket.emit('create:room', { roomName: this.state.roomName }, function (response) {
            if (response.success) {
               alert('방이 성공적으로 개설되었습니다!');
               _this5.setState({ roomExists: true });
            } else {
   ...
   handleJoinRoom: function handleJoinRoom(roomName) {
      var _this6 = this;

      socket.emit('join:room', { roomName: roomName }, function (response) {
         if (response.success) {
            alert('방에 성공적으로 참여했습니다!');
            _this6.props.onRoomJoin(roomName);
         } else {
   ...
   render: function render() {
      ...
      return React.createElement(
         ...
         React.createElement(
            'div',
            { className: 'search_room_top' },
            React.createElement('input', { className: 'search_room_input',
               placeholder: '찾을 방',
               onChange: this.handleChange,
            ...
            React.createElement(
               'button',
               { onClick: this.handleSearch, className: 'search_button' },
               React.createElement('img', { src: 'images/magnifier_icon.png', alt: '검색' })
            )
         ),
         this.state.roomExists === false && React.createElement(
            'div',
            { className: 'search_room_check' },
            React.createElement(
               'p',
               null,
            ...
            React.createElement(
               'button',
               { onClick: function () {
                     return _this7.setState({ roomExists: null });
                  } },
               'X'
            )
         ),
         this.state.roomExists === true && React.createElement(
            'div',
            null,
            React.createElement(
               'div',
               null,
               this.state.roomList.map(function (room, index) {
                  return React.createElement(
                     'div',
                     { className: 'search_room_exist', key: index, onClick: function () {
                           return _this7.handleJoinRoom(room);
                        } },
                     React.createElement('img', { src: 'images/home_icon.png', alt: 'room' }),
                     React.createElement(
                        'p',
                        null,
                        room
...
var ChatApp = React.createClass({
   ...
   handleMessageSubmit: function handleMessageSubmit(message) {
      ...
      messages.push(message);
      this.setState({ messages: messages });
      socket.emit('send:message', _extends({}, message, { user: this.state.user, roomName: this.state.currentRoom }));
   },
   ...
   handleSwitchToSignUp: function handleSwitchToSignUp() {
      this.setState({ signUp: false });
   },

   handleSwitchToLogin: function handleSwitchToLogin() {
      this.setState({ signUp: true });
   },

   handleRoomJoin: function handleRoomJoin(roomName) {
      var _this9 = this;

      socket.emit('join:room', { roomName: roomName }, function (response) {
         if (response.success) {
            _this9.setState({ currentRoom: roomName, messages: response.messages });
         } else {
            alert('방 참여 실패: ' + response.message);
         }
      });
   ...
   render: function render() {
      if (!this.state.loggedIn) {
         return this.state.signUp ? React.createElement(LoginForm, { onLogin: this.handleLogin, onSwitchToSignUp: this.handleSwitchToSignUp }) : React.createElement(SignUpForm, { onSignUp: this.handleSignUp, onSwitchToLogin: this.handleSwitchToLogin });
      }
      ...
      return React.createElement(
         ...
         React.createElement(
            ...
            React.createElement(SearchRoom, { onRoomJoin: this.handleRoomJoin })
         ),
         React.createElement(
            'div',
            ...
            this.state.currentRoom && React.createElement(
               'div',
               null,
               React.createElement(MessageName, { msgname: this.state.currentRoom }),
